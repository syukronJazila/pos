...
class AdjustmentsDataTable extends DataTable
{
    ...
    public function dataTable($query) {
        return datatables()
            ...
            ->addColumn('product_name', function ($data) {
                // Misalnya, menggabungkan nama semua produk dalam satu string
                return $data->adjustedProducts->pluck('product.product_name')->join(', ');
            })
            ->addColumn('action', function ($data) {
                return view('adjustment::partials.actions', compact('data'));
            });
    ...
    public function query(Adjustment $model) {
        return $model->newQuery()
            ->with(['adjustedProducts.product']) // Pastikan ini memuat data produk terkait
